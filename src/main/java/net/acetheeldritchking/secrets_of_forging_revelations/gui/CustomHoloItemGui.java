package net.acetheeldritchking.secrets_of_forging_revelations.gui;
...
//I don't want this to copy HoloItemGui, but it works well bruh
public class CustomHoloItemGui extends HoloItemGui {
    ...
    public CustomHoloItemGui(int x, int y, IModularItem item, int textureIndex, Runnable onSelect, Consumer<String> onSlotSelect) {
        this(x, y, item, item.getDefaultStack(), textureIndex, onSelect, onSlotSelect);
    }

    public CustomHoloItemGui(int x, int y, IModularItem item, ItemStack itemStack, int textureIndex, Runnable onSelect, Consumer<String> onSlotSelect) {
        super(x, y, item, textureIndex, onSelect, onSlotSelect);
        ...
        ResourceLocation textureLocation = new ResourceLocation("tetra", "textures/gui/sofr_holo.png");
        this.icon = new GuiTexture(0, 0, 38, 38, 38 * (textureIndex % 6), 218 - 38 * (textureIndex / 6), textureLocation);
    ...
    @Override
    public CustomHoloItemGui setAttachment(GuiAttachment attachment) {
    ...
    public boolean onMouseClick(int x, int y, int button) {
        for(int i = this.elements.size() - 1; i >= 0; --i) {
            if (this.elements.get(i).isVisible() && this.elements.get(i).onMouseClick(x, y, button)) {
                return true;
            }
        }
